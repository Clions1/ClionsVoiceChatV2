...
io.on('connection', (socket) => {
    ...
    socket.on('joinServer', async ({ userId, username, serverId }) => {
        ...
        for (const uid of serverRooms.get(serverId)) {
            if (uid !== userId) {
                ...
                if (username) {
                    usersInRoom.push({ userId: uid, username });
                }
            }
        }
    ...
    // Kullanıcı server'dan ayrıldı
    socket.on('leaveServer', ({ userId, serverId }) => {
        console.log(`${userId} ID'li kullanıcı ${serverId} ID'li server'dan ayrıldı`);
        
        // Server odasından çıkar
        if (serverRooms.has(serverId)) {
            serverRooms.get(serverId).delete(userId);
    ...
    socket.on('disconnect', () => {
        ...
        // Kullanıcıyı bul ve server'lardan çıkar
